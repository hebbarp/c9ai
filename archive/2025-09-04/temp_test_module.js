...
function detectCodeBlocks(text) {
  ...
  // Match markdown code blocks first (trust markdown)
...
function generateCodeButtons(codeBlock, blockIndex) {
  ...
  const isExecutable = ['python', 'javascript', 'bash', 'powershell', 'sh'].includes(codeBlock.language.toLowerCase());
    ...
    if (toolName === "jit") {
      ...
      if (toolResult.type === 'language_generation') {
        if (toolResult.success === true) {
          ...
          output += `| **Filename** | ${toolResult.saveOptions.filename} |\n`;
          output += `| **Executable** | ${toolResult.saveOptions.executable ? '✅ Yes' : '❌ No'} |\n\n`;
